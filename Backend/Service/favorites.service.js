...
//######################################################################## GET ALL ########################################################################
async function getAllFavoritesService({
  limit,
  offset,
  orderBy,
  orderAttr,
  ...
  const builder = new TableBuilder("favorites").select("*");
...
// ######################################################################## GET ONE  ########################################################################
async function getFavoritesByIdService(id, { fields } = {}) {
  const builder = new TableBuilder("favorites").select("*");
...
//######################################################################## UPDATE ########################################################################
async function updateFavoritesService(id, data) {
  const query = new TableBuilder("favorites")
    .update(data)
    .where("id", "=", id)
    .build();
...
//######################################################################## SOFT DELETE ########################################################################
async function deleteFavoritesService(id) {
  const builder = new TableBuilder("favorites").update({ deleted: true });
  ...
  return {
    message: id
      ? `L'élément avec l'id ${id} de la table favorites a bien été supprimé`
      : `Tous les éléments de la table favorites ont bien été supprimés`,
    count: result.rows.length,
  };
}

module.exports = {
  insertFavoritesService,
  getAllFavoritesService,
  getFavoritesByIdService,
