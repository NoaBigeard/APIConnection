...
//console.log("Dans le fichier addresses")
// ######################################################################## INSERT ########################################################################
async function insertAddressesService(data) {
  if (data.password) {
    data.password = await bcrypt.hash(data.password, 10);
  }
  await addUuidIfNeeded("addresses", data);
  const insertQuery = new TableBuilder("addresses").insert(data).build();
  const result = await pool.query(insertQuery.query, insertQuery.parameters);
  return cleanFields(result.rows[0]);
}

//######################################################################## GET ALL ########################################################################
async function getAllAddressesService({
  limit,
  offset,
  orderBy,
  orderAttr,
  ...
  const builder = new TableBuilder("addresses").select("*");
...
// ######################################################################## GET ONE  ########################################################################
async function getAddressesByIdService(id, { fields } = {}) {
  const builder = new TableBuilder("addresses").select("*");
...
//######################################################################## UPDATE ########################################################################
async function updateAddressesService(id, data) {
...
//######################################################################## SOFT DELETE ########################################################################
async function deleteAddressesService(id) {
  const builder = new TableBuilder("addresses").update({ deleted: true });
  ...
  return {
    message: id
      ? `L'élément avec l'id ${id} de la table addresses a bien été supprimé`
      : `Tous les éléments de la table addresses ont bien été supprimés`,
    count: result.rows.length,
  };
}

module.exports = {
  insertAddressesService,
