...
//console.log("Dans le fichier sizes")
// ######################################################################## INSERT ########################################################################
async function insertSizesService(data) {
  if (data.password) {
    data.password = await bcrypt.hash(data.password, 10);
  }
  await addUuidIfNeeded("sizes", data);
  const insertQuery = new TableBuilder("sizes").insert(data).build();
  const result = await pool.query(insertQuery.query, insertQuery.parameters);
  return cleanFields(result.rows[0]);
}

//######################################################################## GET ALL ########################################################################
async function getAllSizesService({
  limit,
  offset,
  orderBy,
  orderAttr,
  ...
  const builder = new TableBuilder("sizes").select("*");
...
// ######################################################################## GET ONE  ########################################################################
async function getSizesByIdService(id, { fields } = {}) {
  const builder = new TableBuilder("sizes").select("*");
...
//######################################################################## UPDATE ########################################################################
async function updateSizesService(id, data) {
  const query = new TableBuilder("sizes")
    .update(data)
    .where("id", "=", id)
    .build();
...
//######################################################################## SOFT DELETE ########################################################################
async function deleteSizesService(id) {
  const builder = new TableBuilder("sizes").update({ deleted: true });
  ...
  return {
    message: id
      ? `L'élément avec l'id ${id} de la table sizes a bien été supprimé`
      : `Tous les éléments de la table sizes ont bien été supprimés`,
